...
for(const invalid of ['storage_api: 1\r\n',run.stdout.match(/storage_api: 1\r?\n[\s\S]*?storage_end: 1\r?\n/)[0].replace('schema: 1','schema: 2')]){if(invalid!==undefined)assert.throws(()=>parseStorage(invalid));}
assert.throws(()=>parseConfigurationExport(backups[3].raw.replace('# schema: 1','# schema: 2'),'dump'));

# Explicit props-off closed-loop development profile. Enables control outputs, not flight qualification.
# Requires -PropsRemoved -AcknowledgeUnqualifiedFirmware. Never flashes a controller.
param(
 [ValidateSet('kakute_f7_hdv')][string]$Board = 'kakute_f7_hdv',
 [switch]$PropsRemoved,
 [switch]$AcknowledgeUnqualifiedFirmware
)
if (!$PropsRemoved -or !$AcknowledgeUnqualifiedFirmware) {
 throw 'Explicit props-removed and unqualified-firmware acknowledgments are required. This software cannot detect attached props.'
}
$mcu = if ($Board -eq 'kakute_f7_hdv') { 'STM32F745' } else { 'STM32F722' }
$toolchain = if ($Board -eq 'kakute_f7_hdv') { 'cmake/stm32f745.cmake' } else { 'cmake/stm32f722.cmake' }
...
$build = Join-Path $source ("build-closed-loop-development-" + $Board)
...
& $cmake -S $source -B $build -G 'MinGW Makefiles' ("-DCMAKE_TOOLCHAIN_FILE=" + $toolchain) ("-DBOBFLIGHT_BOARD=" + $Board) '-DBOBFLIGHT_HOST_SMOKE=OFF' '-DBOBFLIGHT_FLIGHT_ENABLE=ON' '-DBOBFLIGHT_ACCEL_BENCH_RELAXED=OFF' '-DBOBFLIGHT_PROVE_RESET=OFF' '-DBOBFLIGHT_BOOT_LED_DIAGNOSTICS=OFF'
...
$hex = Join-Path $repo ("bobflight-" + $Board + "-closed-loop-development.hex")
Copy-Item -LiteralPath (Join-Path $build 'bobflight.hex') -Destination $hex -Force
Get-FileHash -Algorithm SHA256 -LiteralPath $hex
Write-Host 'BUILD CHECKPOINT: experimental control-output image built; NOT flight-qualified. No controller was flashed.'
$expectedVersion = '0.2.0-prototype-switchbench2-bl1-calstore2-piddiag2' # matches version.h; the dev image identifies via status flight_mode
Write-Host "Expected firmware: $expectedVersion; board $Board."
Write-Host 'Read the closed-loop development acceptance/recovery guide before installation. Control outputs CAN arm if the normal pre-arm checks pass. Keep props removed. No hardware qualification is implied.'

# BL bench build; Holybro first. Never flashes a controller.
param([ValidateSet('kakute_f7_hdv','tmotor_f7_v2')][string]$Board = 'kakute_f7_hdv')
...
$build = Join-Path $source ("build-bootloader-" + $Board)
...
& $cmake -S $source -B $build -G 'MinGW Makefiles' ("-DCMAKE_TOOLCHAIN_FILE=" + $toolchain) ("-DBOBFLIGHT_BOARD=" + $Board) '-DBOBFLIGHT_HOST_SMOKE=OFF' '-DBOBFLIGHT_FLIGHT_ENABLE=OFF' '-DBOBFLIGHT_ACCEL_BENCH_RELAXED=OFF' '-DBOBFLIGHT_PROVE_RESET=OFF' '-DBOBFLIGHT_BOOT_LED_DIAGNOSTICS=OFF'
...
$hex = Join-Path $repo ("bobflight-" + $Board + "-bootloader-bench.hex")
...
$expectedVersion = if ($Board -eq 'kakute_f7_hdv') { '0.2.0-prototype-switchbench2-bl1-calstore1' } else { '0.2.0-prototype-tmotorf7v2-sensor2-bl1-calstore1' }

...
# Build only; never flashes a board or enables flight.
...
if (!(Test-Path -LiteralPath (Join-Path $armDir 'arm-none-eabi-gcc.exe'))) { throw 'ARM compiler not found. Install Arm GNU Toolchain or restore the existing tools folder.' }
$env:PATH = $armDir + ';C:\TDM-GCC-64\bin;' + $env:PATH
$source = Join-Path $repo 'bobflight-firmware'
$build = Join-Path $source 'build-ports-modes-kakute'
& $cmake -S $source -B $build -G 'MinGW Makefiles' '-DCMAKE_TOOLCHAIN_FILE=cmake/stm32f745.cmake' '-DBOBFLIGHT_BOARD=kakute_f7_hdv' '-DBOBFLIGHT_FLIGHT_ENABLE=OFF' '-DBOBFLIGHT_ACCEL_BENCH_RELAXED=OFF' '-DBOBFLIGHT_BOOT_LED_DIAGNOSTICS=OFF'
if ($LASTEXITCODE -ne 0) { throw 'Firmware configuration failed.' }
& $cmake --build $build -j 4
if ($LASTEXITCODE -ne 0) { throw 'Firmware build failed.' }
$hex = Join-Path $repo 'bobflight-kakute-f7-hdv-ports-modes-bench.hex'
Copy-Item -LiteralPath (Join-Path $build 'bobflight.hex') -Destination $hex
Get-FileHash -LiteralPath $hex -Algorithm SHA256
Write-Host "Built bench-only HEX: $hex"
Write-Host 'Not flashed. Keep props removed. This build does not validate flight safety.'

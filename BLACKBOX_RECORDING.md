...
- **No integrated configurator file listing/download or USB mass-storage mode yet.** Read-only PC USB extraction is now documented in [USB-LOG-EXPORT.md](USB-LOG-EXPORT.md), including files from earlier power cycles. A powered-off card reader remains the fallback. `error` is not a successful close.

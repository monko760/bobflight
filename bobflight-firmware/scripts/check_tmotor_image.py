...
"""Bounded Intel HEX validation for the F722 sensor image; no device access."""
...
def validate(path):
    ...
    for number,line in enumerate(Path(path).read_text().splitlines(),1):
        ...
        if kind==0:
            ...
            for offset,value in enumerate(data):
                ...
                if not 0x08000000<=at<0x08080000:raise ValueError('Data outside F722 512 KiB program flash')
    ...
    if sp!=0x20010000 or sp%8:raise ValueError('Wrong F722 initial stack pointer')
    ...
    for marker in [b'tmotor_f7_v2',b'0.2.0-prototype-tmotorf7v2-sensor1']:
...
if __name__=='__main__':
    try:
        count,end,digest=validate(sys.argv[1])
        print(f'PASS TMOTORF7V2 HEX: {count} programmed bytes, end=0x{end:08x}, F722 vectors/bounds/version; SHA256={digest}')
    except (ValueError,KeyError,OSError,IndexError) as exc:
        sys.exit(f'TMOTORF7V2 HEX rejected: {exc}')

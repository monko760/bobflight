...
int main(void){
 config_init();pid_diag_init();rc[4]=-1.f;
 cycle(1000);assert(pid_diag_start(PID_DIAG_SOURCE_ZERO,NULL));
 cycle(1000);assert(!snapshot().valid);cycle(1000);
 pid_diag_snapshot_t s=snapshot();assert(s.valid&&s.dt_us==1000&&s.correction[0]<0&&s.correction[1]>0);
 // Bad gravity does not gate rate diagnostics, and still does not permit flight.
 assert(arm==ARM_DISARMED);for(unsigned j=0;j<4;j++)assert(motors[j]==0.f);
 for(unsigned i=0;i<25;i++){cycle(1000);assert(snapshot().valid);for(unsigned j=0;j<4;j++)assert(motors[j]==0.f);}
 assert(motor_calls==28); // Only existing cascade sends its ordinary zero frames.
 cycle(20000);assert(!snapshot().valid);cycle(1000);assert(!snapshot().valid);cycle(1000);assert(snapshot().valid);
 usb=false;cycle(1000);assert(!snapshot().active);usb=true;cycle(1000);assert(!snapshot().active);
 assert(pid_diag_start(PID_DIAG_SOURCE_ZERO,NULL));cycle(1000);cycle(1000);calibrating=true;cycle(1000);assert(!snapshot().active);calibrating=false;
 // This call exercises mutual exclusion in fake IO only, never on hardware.
 assert(pid_diag_start(PID_DIAG_SOURCE_ZERO,NULL));assert(bench_motor_pulse(1,8));
 cycle(1000);assert(!snapshot().active);assert(bench_motor_test(0));cycle(1000);
 for(unsigned j=0;j<4;j++)assert(motors[j]==0.f);
 assert(arm==ARM_DISARMED);puts("PASS real task cascade + shadow PID: bad-accel independence, measured dt, existing zero output, disconnect/calibration/motor exclusion; no physical motor test");return 0;
}

...
def fixture(sp=0x20010000,reset=0x08000009,marker=b'tmotor_f7_v2\0' b'0.2.0-prototype-tmotorf7v2-sensor1\0'):
 return [rec(0,4,b'\x08\x00'),rec(0,0,struct.pack('<II',sp,reset)),rec(8,0,marker),rec(0,1,b'')]
with tempfile.TemporaryDirectory() as td:
 p=Path(td)/'synthetic.txt'
 def check(lines,valid):
  p.write_text('\n'.join(lines)+'\n')
  try:m.validate(p)
...
print('PASS synthetic F722 HEX validator: correct markers/vectors/bounds, wrong stack/reset/target/entry, missing or duplicate EOF, overlap, checksum rejected')

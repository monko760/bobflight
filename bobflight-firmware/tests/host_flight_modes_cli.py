...
def verify(text):
 ...
 assert 'flight_enabled: 1' in text
...
# Unified build: manual selection only; AUX source is refused, ranges remain editable storage.
text=run('modes\ncontrol_source aux\nmode_range HORIZON 1 2 1301 1700\nmode_range ACRO 1 2 1701 2100\ncontrol_mode horizon\nmodes\n')
ss=snapshots(text);assert len(ss)==4,text
for snap in ss:verify(snap)
assert 'control_source: manual' in ss[0] and 'requested_mode: angle' in ss[0]
assert 'control_source refused' in text
assert 'control_source: manual' in ss[1] # aux stays retired
assert 'mode: HORIZON,1,2,1301,1700,0' in ss[1]
assert 'requested_mode: horizon' in ss[-1]
assert 'mode: HORIZON,1,2,1301,1700,0' in ss[-1]
assert 'mode: ACRO,1,2,1701,2100,0' in ss[-1]
for bad in ['control_source aux','control_source AUX','control_source aux extra','control_source auto','control_source 1','control_mode level extra','mode_range HORIZON 1 13 1000 2000','mode_range ACRO 1 2 1500 1500']:
 t=run(bad+'\nmodes\n');assert 'refused' in t,(bad,t);assert 'control_source: manual' in snapshots(t)[-1]
...
print('PASS unified flight modes CLI: v2 framing, four rows, manual-only source, horizon manual, editable ranges, guards, cold-start defaults')

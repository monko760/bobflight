...
static bool acceptable(const uint8_t*p,uint32_t board,size_t n,unsigned version){
 return version>1 ? (version==3&&record_valid(p,board,160,3))||record_valid(p,board,128,2)||record_valid(p,board,96,1) : valid(p,board,n);
}
...
static bool read_slot(uint32_t offset,uint8_t *p,size_t n,unsigned version){
 ...
 size_t tail=0;
 if(rd(p)==MAGIC){
  if(version==3&&rd(p+4)==3&&rd(p+12)==160)tail=160+COMMIT_BYTES;
...
static config_store_result_t save(uint32_t board,const void *payload,size_t n,unsigned version){
 ...
 for(unsigned i=0;i<2;i++){const uint8_t*p=i?b:a;if(rd(p)==MAGIC&&(rd(p+8)!=board || (version>1 ? !((rd(p+4)==1&&rd(p+12)==96)||(rd(p+4)==2&&rd(p+12)==128)||(version==3&&rd(p+4)==3&&rd(p+12)==160)) : (rd(p+4)!=1||rd(p+12)!=n))))return CONFIG_STORE_INVALID;}

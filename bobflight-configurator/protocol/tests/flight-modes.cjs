...
const {parseModes,MockPortsModes,isControlSourceCommand,controlModeCommand,isControlModeCommand,canEditModeRanges,canSelectControlSource,controlSourceCommand,canSelectControlMode,ResponseCollector,BobFlightCliClient,MockTransportFactory}=P;
async function main(){
 ...
 assert.equal(s.apiVersion,2);assert.equal(s.modes.length,4);assert.equal(s.armSemantics,'preview');assert.equal(s.requestedMode,'angle');assert.equal(s.controlSource,'manual');assert.equal(s.flightEnabled,true);
 ...
 assert.throws(()=>parseModes(conflict),/AUX mode routing is unavailable/); /* unified firmware refuses aux snapshots */
 const manual=raw.replace('mode_conflict: 0','mode_conflict: 1');
 assert.throws(()=>parseModes(manual),/conflict/); /* manual routing can never be in conflict */
 assert.equal(isControlSourceCommand('control_source manual'),true);assert.equal(isControlSourceCommand('control_source aux'),true);
...
assert.equal(parseModes(mock.handle('control_source manual',false,false)).controlSource,'manual');
for(const cmd of ['control_mode angle','control_mode acro','control_mode horizon']){assert.equal(isControlModeCommand(cmd),true);assert.equal(parseModes(mock.handle(cmd,false,false)).requestedMode,cmd.split(' ')[1]);}
for(const cmd of ['control_mode level','control_mode acro extra','control_mode angle\narm','control_mode  acro','control_mode auto']){assert.equal(isControlModeCommand(cmd),false);assert.match(mock.handle(cmd,false,false),/refused/);}
assert.equal(controlModeCommand('horizon'),'control_mode horizon');assert.throws(()=>controlModeCommand('level'));
 for(const cmd of ['control_source AUX','control_source aux extra','control_source aux\narm','control_source  aux','control_source auto']){assert.equal(isControlSourceCommand(cmd),false);assert.match(mock.handle(cmd,false,false),/refused/);}
 assert.equal(controlSourceCommand('aux'),'control_source aux');assert.throws(()=>controlSourceCommand('auto'));
 assert.match(mock.handle('control_source aux',true,false),/refused/);assert.match(mock.handle('control_mode horizon',true,false),/refused/);assert.match(parseModes(mock.handle('control_mode horizon',false,false)).raw,/requested_mode: horizon/);
 assert.equal(canEditModeRanges(s,true,false),true);assert.equal(canSelectControlSource(s,true,false),false); /* unified flight firmware: aux retired */assert.equal(canSelectControlMode(s,true,false),true);
 for(const [snapshot,connected,pending] of [[null,true,false],[s,false,false],[s,true,true],[{...s,armed:true},true,false],[{...s,benchActive:true},true,false],[{...s,calibrationActive:true},true,false]]){
  assert.equal(canEditModeRanges(snapshot,connected,pending),false);assert.equal(canSelectControlSource(snapshot,connected,pending),false);assert.equal(canSelectControlMode(snapshot,connected,pending),false);
 }
 assert.equal(canSelectControlSource({...s,flightEnabled:true},true,false),false);
assert.equal(canSelectControlMode({...s,flightEnabled:true},true,false),true); /* manual mode selection survives flight enablement */
 ...
 assert.equal(parseModes(await client.sendCommand('control_mode horizon')).requestedMode,'horizon');
 ...
 function page(snapshot,connected=true,pending=false){
  const calls=[];
  const query={snapshot,connected,pending,error:'',loadedAt:1,execute:async(cmd,verify)=>{calls.push(cmd);const v={...snapshot,requestedMode:cmd.startsWith('control_mode ')?cmd.slice(13):snapshot.requestedMode};verify?.(v);return v;}};
 ...
 assert.match(text(ready.tree),/AUX mode routing is retired/); /* unified flight firmware */
 const acro=nodes(ready.tree).find(n=>n.type==='button'&&text(n)==='Acro');assert.equal(acro.props.disabled,false);await acro.props.onClick();assert.deepEqual(ready.calls,['control_mode acro']);
 const horizon=nodes(ready.tree).find(n=>n.type==='button'&&text(n)==='Level \(Horizon\)');assert.equal(horizon.props.disabled,false);await horizon.props.onClick();assert.deepEqual(ready.calls,['control_mode acro','control_mode horizon']);
 const angle=nodes(ready.tree).find(n=>n.type==='button'&&text(n)==='Angle');assert.equal(angle.props.disabled,true); /* already requested */
 for(const [snap,connected,pending] of [[s,false,false],[s,true,true],[{...s,armed:true},true,false],[{...s,benchActive:true},true,false],[{...s,calibrationActive:true},true,false]]){
  const p=page(snap,connected,pending),b=nodes(p.tree).find(n=>n.type==='button'&&text(n)==='Acro');
  if(b){assert.equal(b.props.disabled,true);await b.props.onClick();}assert.deepEqual(p.calls,[]);
 }
 const disconnected=page(s,false);assert.doesNotMatch(text(disconnected.tree),/Last snapshot:/);
 assert.match(text(page(old).tree),/Legacy firmware/);assert.equal(nodes(page(old).tree).some(n=>n.type==='button'&&text(n)==='Acro'),false);
 console.log('PASS unified flight modes: API2/legacy strict parsing, retired AUX refused, manual angle/acro/horizon allowlist and client roundtrip, real StoragePanel import plus ModesPage guard/readback/disconnect wiring');
}

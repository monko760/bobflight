...
export function parseConfigurationExport(raw:string,kind:'diff'|'dump'):ConfigurationExport {
 ...
 for(const line of lines.slice(1,-1)){
  if(line.startsWith('# ')){if(commands)throw Error('Misplaced export header');const m=/^# ([a-z_]+): (.+)$/.exec(line);if(!m||headers.has(m[1]))throw Error('Malformed export header');headers.set(m[1],m[2]);}
    ...
    if(headers.get('schema')!=='3'&&headers.get('schema')!=='4'&&headers.get('schema')!=='5'&&headers.get('schema')!=='6')throw Error('Unexpected power/motor setting');
 ...
 const excludes=headers.get('schema')==='6'||headers.get('schema')==='5'||headers.get('schema')==='4'||headers.get('schema')==='3'?'gyro_calibration':headers.get('schema')==='2'?'gyro_calibration,power,dshot':'calibration,power,dshot';

...
const scopeFor=(v:string|undefined)=>v==='4'?STORAGE_SCOPE_V4:v==='3'?STORAGE_SCOPE_V3:v==='2'?STORAGE_SCOPE_V2:STORAGE_SCOPE;
export interface StorageSnapshot {backend:'flash'|'host_sim'|'unsupported';schema:1|2|3|4;state:'saved'|'dirty'|'defaults'|'error'|'unsupported';dirty:boolean;generation:number;lastError:string;scope:string;armed:boolean;benchActive:boolean;calibrationActive:boolean;flightEnabled:boolean}
export function parseStorage(raw:string):StorageSnapshot {
 ...
 if(!['flash','host_sim','unsupported'].includes(backend)||!['saved','dirty','defaults','error','unsupported'].includes(state)||!(['1','2','3','4'].includes(field(f,'schema')))||field(f,'scope')!==scopeFor(field(f,'schema')))throw Error('Unsupported storage capability/schema');
 ...
 return {backend:backend as StorageSnapshot['backend'],schema:Number(field(f,'schema')) as 1|2|3|4,state:state as StorageSnapshot['state'],dirty,generation:uint(field(f,'generation'),0,4294967295),lastError,scope:field(f,'scope'),armed:bit(field(f,'armed')),benchActive:bit(field(f,'bench_active')),calibrationActive:bit(field(f,'calibration_active')),flightEnabled:bit(field(f,'flight_enabled'))};
}
...
export function parseConfigurationExport(raw:string,kind:'diff'|'dump'):ConfigurationExport {
 ...
 for(const line of lines.slice(1,-1)){
  if(line.startsWith('# ')){if(commands)throw Error('Misplaced export header');const m=/^# ([a-z_]+): (.+)$/.exec(line);if(!m||headers.has(m[1]))throw Error('Malformed export header');headers.set(m[1],m[2]);}
    ...
    if(headers.get('schema')!=='3'&&headers.get('schema')!=='4')throw Error('Unexpected power/motor setting');
    ...
    if(line.startsWith('set ')){const [,key,value]=line.split(' ');const n=Number(value);if(!isSettingsKey(key)||!Number.isFinite(n)||(key.startsWith('rate_max_')?(n<10||n>2000):key==='rate_expo'?(n<0||n>1):key==='min_throttle'?(n<0||n>0.2):key==='airmode'?(n!==0&&n!==1):(n<0||n>10)))throw Error('Invalid exported setting');}
  }
 }
 const board=headers.get('board')??'',firmware=headers.get('firmware')??'',count=headers.get('mode_count');
 const excludes=headers.get('schema')==='4'||headers.get('schema')==='3'?'gyro_calibration':headers.get('schema')==='2'?'gyro_calibration,power,dshot':'calibration,power,dshot';
 if(!['1','2','3','4'].includes(headers.get('schema')??'')||headers.get('kind')!==kind||headers.get('scope')!==scopeFor(headers.get('schema'))||headers.get('excludes')!==excludes||!['2','4'].includes(count??'')||!/^[-a-zA-Z0-9_]+$/.test(board)||!/^[-a-zA-Z0-9._+]+$/.test(firmware))throw Error('Unsupported export identity/schema');

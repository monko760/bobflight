...
export interface ParsedPorts{raw:string;board:string;receiverUart:number;rebootRequired:boolean;persistence:'session';armed:boolean;benchActive:boolean;ports:PortRow[];source?:string}
export function parsePorts(raw:string):ParsedPorts{
 const {fields:f,rows}=snapshot(raw,'ports','port');const board=field(f,'board');if(!/^[a-zA-Z0-9_.-]+$/.test(board))throw Error('Invalid board ID');if(field(f,'persistence')!=='session')throw Error('Unsupported persistence');const rr=field(f,'reboot_required');if(rr!=='no'&&rr!=='yes')throw Error('Invalid reboot requirement');const receiverUart=uint(field(f,'receiver_uart'),0,255);
 ...
 return {raw,board,receiverUart,rebootRequired:rr==='yes',persistence:'session',armed:bit(field(f,'armed')),benchActive:bit(field(f,'bench_active')),ports,source:f.get('source')};
}

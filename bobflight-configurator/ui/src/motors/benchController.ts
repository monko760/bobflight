 ...
 */
import type { BobFlightHost, CliCommand, ParsedStatus } from "../protocol/types";
...
export interface BenchCapabilities { individual: boolean; sequence: boolean; dshot: boolean }
export function parseBenchHelp(text: string): BenchCapabilities {
  const has = (name: string) => new RegExp(`^\\s*${name}(?:\\s|$)`, "m").test(text);
  return { individual: has("motor_test"), sequence: has("motor_seq"), dshot: has("dshot") };
}
...
export interface BenchState {
  ...
  propsOff: boolean; stationary: boolean; busy: boolean; stopping: boolean;
...
const initial = (): BenchState => ({ connected: false, visible: true, status: null, statusAt: -Infinity,
  capabilities: null, rate: null, propsOff: false, stationary: false, busy: false, stopping: false,
  estimatedUntil: 0, testLabel: "", reply: "", error: "" });
...
export class BenchController {
  ...
  setVisible(visible: boolean): void {
    this.patch({ visible });
    if (!visible) { this.patch({ propsOff: false, stationary: false }); void this.stop(); }
  }
  leave(): Promise<void> {
    this.patch({ visible: false, propsOff: false, stationary: false });
    this.generation++;
  ...
  private run(work: (g: number) => Promise<void>): Promise<void> {
    ...
    const p = Promise.resolve().then(async () => { if (this.valid(g)) await work(g); })
      .catch(e => { if (this.valid(g)) this.patch({ error: e instanceof Error ? e.message : String(e), status: null, rate: null, propsOff: false, stationary: false }); })
      .finally(() => { if (this.flight === p) { this.flight = null; if (!this.stopFlight) this.patch({ busy: false }); } });
    this.flight = p;
    return p;
  }
  poll(): Promise<void> {
  ...
  start(motor: 1 | 2 | 3 | 4 | "sequence"): Promise<void> {
    if (benchBlockReason(this.state, this.now()) || !(motor === "sequence" ? this.state.capabilities?.sequence : this.state.capabilities?.individual)) return Promise.resolve();
    return this.run(async g => {
      ...
      const cmd: CliCommand = motor === "sequence" ? "motor_seq" : `motor_test ${motor}`;
      const duration = motor === "sequence" ? 7000 : 1200;
      this.possibleMotion = true;
      this.patch({ stationary: false, estimatedUntil: this.now() + duration, testLabel: motor === "sequence" ? "Sequence M1 → M2 → M3 → M4" : `Motor ${motor}`, error: "", reply: "Sending test request…" });
  ...
  stop(): Promise<void> {
    ...
    this.patch({ busy: true, stopping: true, stationary: false });
    ...
    const p = Promise.resolve(previous).then(async () => {
      ...
      if (this.stopFlight === p) { this.stopFlight = null; this.patch({ busy: false, stopping: false }); }
    });
    this.stopFlight = p;
    return p;
  }
